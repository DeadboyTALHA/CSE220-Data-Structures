...
Original file is located at
    https://colab.research.google.com/drive/12BDTvV-aJegxnQH52ieksczJAk3rdUVL
"""
...
class VehicleHashTable:
  ...
  def insert_vehicle(self, vehicle):
    #TO DO
    key = self.__hash_function(vehicle[0])
    node = vehicleNodes(vehicle[0], vehicle[1], vehicle[2], vehicle[3])
    if self.vehicleTable[key] == None:
      self.vehicleTable[key] = node
    elif self.vehicleTable[key].brand == node.brand:
      node.next = self.vehicleTable[key]
    else:
      key = self.find_empty_slot()
    self.vehicleTable[key] = node
...
class Hashtable:
  ...
  def __hash_function(self, key):
    ...
    for i in range(0, len(key), 2):
      sum += int(str(ord(key[i])) + str(ord(key[i+1])))
  ...
  def search_hashtable(self, s):
    ...
    tail = self.ht[idx]
    while tail != None:
      if tail.key == s[0] and tail.value == s[1]:
        return 'Found'
      tail = tail.next
    return 'Not Found'
...
class HashTable:
  ...
  def insert(self, key, value):
    ...
    if self.ht[idx].value == None:
      ...
      tail = self.ht[idx]
      if node.value[1] > tail.value[1]:
        node.next = tail
        self.ht[idx] = node
      else:
        while tail.next != None and tail.next.value[1] >= node.value[1]:
          tail = tail.next
        node.next = tail.next
        tail.next = node
